...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	 ...
	 * @param {boolean} [options.auto=true] 设置为`true`后，不需要手动调用上传，有文件选择即开始上传。用this.uploads()来上传，详情请看example
	 ...
	 *
	 *        // this.uploads(); // 如果是手动上传，这里可以通过调用upload来实现；也可以用它来实现重传。
	 *        // this.stop(); // 中断上传
	 *
	        ...
	        img.onload = function () {
	            ...
	            if (options.type == 'file') {
	                if (/;base64,null/.test(dataURL) || /;base64,$/.test(dataURL)) {
	                    ...
	                    console.warn('Compress fail, dataURL is ' + dataURL + '. Next will use origin file to uploads.');
	                    callback(file);
	                } else {
